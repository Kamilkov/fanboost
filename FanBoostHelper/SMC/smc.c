...
kern_return_t SMCWriteSimple(UInt32Char_t key, char *wvalue, io_connect_t conn)
{
    ...
    size_t i, len;
    char c[3];
    /* FanBoost fix vs upstream: validate even-length hex and loop over byte
     * count — the original iterated over strlen() reading wvalue[i*2] past
     * the end of the string. */
    len = strlen(wvalue);
    if (len == 0 || len % 2 != 0 || len / 2 > sizeof(val.bytes))
...
#ifndef FANBOOST_NO_MAIN  /* FanBoost: CLI entry excluded when built into the helper */
